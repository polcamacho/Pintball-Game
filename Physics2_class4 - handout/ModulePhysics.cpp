...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius, bool dynamic, float velocity, float density)
{
	b2BodyDef body;
	if (dynamic)
...
PhysBody* ModulePhysics::CreateChain(int x, int y, int* points, bool dynamic, int size, float velocity)
{
	b2BodyDef body;
	if (dynamic)
		body.type = b2_dynamicBody;
...
update_status ModulePhysics::PostUpdate()
{
	...
	if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT && mouse_joint != nullptr) {
		...
		App->renderer->DrawLine(A.x, A.y, B.x, B.y, 255, 255, 0, 255);
	}
